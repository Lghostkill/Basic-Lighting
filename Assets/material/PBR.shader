		...
		SubShader
		{
			...
			Pass
			{
				...
				fixed4 frag(v2f i) : SV_Target
				{
					...
					fixed3 Final = (kD * albedo + specular) * _LightColor0.xyz * (max(dot(normal, lightDir), 0.0) + 0.5);//反射及diffuse部分整合
			...
			Pass
			{
				...
				fixed4 frag(v2f i) : SV_Target
				{
					...
					fixed3 lightDir = normalize(UnityWorldSpaceLightDir(worldPos));//灯光方向
						...
						fixed3 Final = (kD * albedo + specular) * atten * _LightColor0.xyz * (max(dot(normal, lightDir), 0.0) + 0.5);//反射及diffuse部分整合
